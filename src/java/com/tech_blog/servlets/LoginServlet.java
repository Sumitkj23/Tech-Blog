...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Account_obj user = new Account_obj();
            Account acc;
            acc = user.getUserInfo(email, pass);
            if(acc != null)
            {
                // login success
                HttpSession s = request.getSession();
                s.setAttribute("current_user", acc);
            ...
            {
                // login fail
                Message m = new Message("Invalid Email or Password!", "error", "alert-danger");
                // redirect into login.jsp page
